...
import { ZenlessUid } from "../../types/api";

export default (app: App) =>
    app.group("/zzz/:identifier/:identifier2", (app) =>
        app
            .derive(async ({ params, cacheOptions, status }) => {
                ...
                const data = (await res.json()) as ZenlessUid;

                const avatars = data.PlayerInfo.ShowcaseDetail.AvatarList;

                const characterIndex = avatars.findIndex(
                ...
                return {
                    enkaUrl: `https://enka.network/zzz/${uid}`,
                    image: CachedImage.getUid(
                        uid,
                        0,
                        characterId,
                        avatars[characterIndex],
                        cacheOptions
                    ),
                    characterIndex,
                };
